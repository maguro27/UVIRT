...
class Run(object):
    def __init__(self, config):
        ...
        self.device = torch.device(
            "cuda:%d" % (int(config["GPU1"])) if torch.cuda.is_available() else "cpu"
        )
        print(self.device)

        C = self.config["G"][
            "CONTENT_DIM"
        ]  # The number of channels of the content feature.
        n_mem = (
            C // self.config["N_GROUP"]
        )  # The number of blocks in the coloring matrix: G, The number of elements for each block: n_members^2
        self.mask = self.get_block_diagonal_mask(
            n_mem
    ...
    def init_network(self):
        ...
        G_params = list(self.G_A.parameters()) + list(
            self.G_B.parameters()
        )  # + list(blah)
    ...
    def model_save(self, iteration):
        ...
        torch.save(
            ...
            os.path.join(
                ...
                "G_A_%s_%d.pth" % (self.config["SAVE_NAME"], iteration),
            ),
        )
        torch.save(
            self.G_B.state_dict(),
            os.path.join(
                self.config["MODEL_SAVE_PATH"],
                "G_B_%s_%d.pth" % (self.config["SAVE_NAME"], iteration),
            ),
        )
        torch.save(
            self.D_A.state_dict(),
            os.path.join(
                self.config["MODEL_SAVE_PATH"],
                "D_A_%s_%d.pth" % (self.config["SAVE_NAME"], iteration),
            ),
        )
        torch.save(
            self.D_B.state_dict(),
            os.path.join(
                self.config["MODEL_SAVE_PATH"],
                "D_B_%s_%d.pth" % (self.config["SAVE_NAME"], iteration),
            ),
        )
    ...
    def update_G(self, x_A, x_B, isTrain=True):
        ...
        if isTrain:
            ...
            loss_cross_rec = self.l1_criterion(x_ABA, x_A) + self.l1_criterion(
                x_BAB, x_B
            )
            ...
            loss_coloring_reg = self.reg(
                [coloring_reg_AB, coloring_reg_BA, coloring_reg_ABA, coloring_reg_BAB]
            )
            ...
            self.loss["G/loss_cross_rec"] = (
                self.config["LAMBDA_X_REC"] * loss_cross_rec.item()
            )
            self.loss["G/loss_ae_rec"] = (
                self.config["LAMBDA_X_REC"] * loss_ae_rec.item()
            )
            ...
            self.loss["G/loss_whitening_reg"] = (
                self.config["LAMBDA_W_REG"] * loss_whitening_reg.item()
            )
            self.loss["G/loss_coloring_reg"] = (
                self.config["LAMBDA_C_REG"] * loss_coloring_reg.item()
            )
    ...
    def train(self):
        ...
        for i in range(self.config["START"], self.config["NUM_ITERS"]):
            ...
            try:
                ...
                if (
                    x_A.size(0) != self.config["BATCH_SIZE"]
                    or x_B.size(0) != self.config["BATCH_SIZE"]
                ):
                    x_A, x_B = next(data_iter)
            except:
                data_iter = iter(data_loader)
            ...
            x_B = x_B.to(
                self.device
            )  # Exemplar images corresponding with target labels.

            ### Training ###
            self.update_D(x_A, x_B)
            (x_AB, x_BA) = self.update_G(x_A, x_B)
            ...
            if i % self.config["SAVE_EVERY"] == 0:
                ...
                print(
                    "G/loss_whitening_reg: %.5f" % (self.loss["G/loss_whitening_reg"])
                )
                print("G/loss_coloring_reg: %.5f" % (self.loss["G/loss_coloring_reg"]))

                save_img(
                    [x_A, x_AB, x_B, x_BA], self.config["SAVE_NAME"], i, "train_results"
                )
                self.model_save(i)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (x_A, x_B) in enumerate(data_loader):
                ...
                save_img(
                    [x_A, x_B, x_AB, x_BA], self.config["SAVE_NAME"], i, "test_results"
                )
